...
def run_wizard(index_url: str) -> dict[str, str]:
    ...
    logger.debug("lyrics_source_wizard: fetching index URL %s", index_url)
    try:
    ...
    try:
        ...
        logger.debug("lyrics_source_wizard: selected sample song %r at %s", song_title, song_url)
        ...
        logger.debug("lyrics_source_wizard: analyzing lyrics page with Claude")
        lyrics_result = _analyze_lyrics_page(song_tmp)
        lyrics_js = lyrics_result["lyrics_js"]

        # Step 7: validate lyrics_js
        logger.debug("lyrics_source_wizard: validating lyrics_js")
