...
class TestBuildCoverPrompt:
    ...
    def test_excludes_playlist_title(self):
        """Prompt must NOT contain the playlist title (title is inpainted separately)."""
        prompt = build_cover_prompt(
            description=None,
            track_titles=[],
            artists=[],
...
class TestTrySharedAlbumArt:
    ...
    def test_returns_true_and_saves_cover_when_all_match(self, tmp_path):
        ...
        mock_reframe.assert_called_once_with(shared_art, cover_path, log=None, on_step=None, on_inner=None)
...
class TestReframeAlbumArt:
    def test_saves_cover_when_quality_passes(self, tmp_path):
        ...
        with (
            patch("yoto_lib.providers.together_provider.TogetherAIProvider", return_value=mock_provider),
